...
with open("sentences.txt") as file:
    i = 0 # номер текущей строки в файле и он же номер текущей строки в итоговой матрице весов
    for line in file:
        # То же, что и в строке реального кода
        # lineTokens =[token for token in re.split('[^a-z]' , line.lower()) if token !=""]
        lineTokens = [token for token in re.split('[^a-z]', line.lower()) if token]
        for j in range(wCount):
            if indexes[j] in lineTokens:
                # находим количество вхождений слова в предложении
                # и сохраняем как [i,j] элемент матрицы весов
                matrx[i, j] = lineTokens.count(indexes[j])
            else:
                matrx[i, j] = 0
        i += 1 
